...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "slice correct int",
			args: args{
				v: struct {
					Sl2 []int `validate:"max:-7"`
				}{
					Sl2: []int{-10, -8, -7},
				},
			},
		...
		},
		{
			...
			args: args{
				v: struct {
					Sl1 []string `validate:"max:5"`
					Sl2 []int    `validate:"max:-7; min:-10"`
				}{
					Sl1: []string{"efgh", "abaca"},
					Sl2: []int{-10, -8, -7},
				},
			},
			wantErr: false,
		...
		{
			name: "slice incorrect",
			args: args{
				v: struct {
					Sl1 []string `validate:"in:abacaba,abaracadabra,kek"`
					Sl2 []int    `validate:"in:1,3,5,7;min:2"`
				}{
					Sl1: []string{"abacaba", "kek", "abaracadabra", "yes", "or", "yes"},
					Sl2: []int{1, 3, 9},
				},
			},
			wantErr: true,
